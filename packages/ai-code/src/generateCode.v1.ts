...
const DirectoryStructureSchema: z.ZodType<{
  ...
  children: z
    .array(z.lazy(() => DirectoryStructureSchema))
    .optional()
...
const ChunkDetailSchemaContents = {
  id: z
    .string()
    .describe(
      'A unique identifier for this chunk (e.g., "src/utils/helpers.ts:imports", "src/components/Button.tsx:render"). The ID should be namespaced by the file path.'
    ),
  title: z
    .string()
    .describe(
      'A short, descriptive title for this code chunk (e.g., "Imports", "MyFunction").'
    ),
  description: z
    .string()
    .describe(
  ...
  dependencies: z
    .array(z.string())
    .describe(
      'A list of other chunk IDs that this chunk directly depends on. This is crucial for generation order.'
    ),
  status: z
    .enum(['pending', 'decomposing', 'generating', 'completed', 'failed'])
    .default('pending')
    .describe('The current processing status of this chunk.'),
  code: z
    .string()
    .optional()
    .describe(
      'The final generated code for this chunk. This is only present on leaf nodes.'
    ),
  children: z
    .array(z.lazy(() => ChunkDetailSchema))
    .optional()
    .describe(
      'A list of sub-chunks, if this chunk is complex and needs to be decomposed.'
    ),
};
export const ChunkDetailSchema: z.ZodType<any> = z.object(
  ChunkDetailSchemaContents
);

// A non-recursive version for the AI to use when decomposing.
const SubChunkPlanSchema = z
  .object(ChunkDetailSchemaContents)
  .omit({ status: true, code: true, children: true });
...
export const FileDetailSchema = z.object({
  ...
  fileExtension: z
    .string()
    .describe('The file extension (e.g., "ts", "js", "json").'),
  fullFileName: z
    .string()
    .describe('The complete filename with the extension.'),
  path: z
    .string()
    .describe('The relative path where the file should be created.'),
  type: z
    .enum([
      'typescript',
      'javascript',
  ...
  brief: z
    .string()
    .describe('A brief, one-sentence description of what this file contains.'),
  dependencies: z
    .array(z.string())
    .optional()
    .describe('This is deprecated in favor of chunk-level dependencies.'),
  chunks: z
...
const PlanSchema = z.object({
  files: z
    .array(FileDetailSchema)
    .describe(
      'The detailed list of files to be generated to fulfill the instruction. Only define the TOP-LEVEL chunks for each file.'
    ),
  codeBrief: z
    .string()
    .optional()
    .describe(
      'A brief, one-paragraph summary of what the generated code will do and how the files work together.'
    ),
});
...
export const CodeChunkSchema = z.object({
  code: z
    .string()
    .describe(
      'A chunk of code for the file. This can be a partial piece or the entire file if it is small.'
    ),
  imports: z
    .array(
      z.object({
        module: z
          .string()
          .describe(
            'The module or path to import from (e.g., "react", "./utils").'
          ),
        specifiers: z
          .array(z.string())
          .describe(
            'The named or default imports (e.g., ["useState", "default as React"]).'
          ),
      })
    )
    .optional()
    .describe("A list of imports required by this chunk's code."),
});
...
export const GenerationStateSchema = z.object({
  ...
  fileImports: z
    ...
    .optional()
    .describe('A map of file paths to their required imports.'),
});
...
async function updateFileWithChunk(
  filePath: string,
  chunkId: string,
  code: string
): Promise<string> {
  ...
  if (!originalContent.includes(placeholder)) {
    console.warn(
      `[updateFileWithChunk] Warning: placeholder for chunk "${chunkId}" not found in file "${filePath}". Code was not inserted.`
    );
...
function topologicalSort(allChunks: Map<string, ChunkDetail>): string[] {
  ...
  const visit = (chunkId: string) => {
    if (recursionStack.has(chunkId)) {
      throw new Error(
        `Circular dependency detected in chunks: ${[...recursionStack, chunkId].join(' -> ')}`
      );
    }
    ...
    if (chunk) {
      for (const depId of chunk.dependencies) {
        if (!allChunks.has(depId)) {
          ...
          console.warn(
            `[TopologicalSort] Warning: Chunk "${chunkId}" has an unknown dependency "${depId}". This dependency will be ignored.`
          );
          continue;
        }
        visit(depId);
...
export async function generateCodeV1<TParams = any>(
  ...
  logVerbose(
    verbose,
    logStream,
    '[INFO] Starting project planning phase (Architect)...'
  );
...
Based on the instruction and directory structure, create the high-level generation plan. Remember to only define the top-level, conceptual chunks for each file. Do not create a nested or detailed plan.`;
  ...
  planParams = {
    ...planParams,
    ...('params' in planResult.object && planResult.object.params
      ? planResult.object.params
      : {}),
  };

  logVerbose(verbose, logStream, '[INFO] High-level plan complete.', {
    plan: plan,
    params: planParams,
  });
  ...
  logVerbose(
    verbose,
    logStream,
    '[INFO] Starting chunk planning and decomposition phase (Planner)...'
  );
    ...
    for (const file of files) {
      ...
      for (const topLevelChunk of file.chunks) {
        logVerbose(
          verbose,
          logStream,
          `[INFO] Decomposing top-level chunk: ${topLevelChunk.id}`
        );
...
Please correct your response. It MUST be a single call to the 'submitPlan' tool.`
                ...
                submitPlan: {
                  ...
                  parameters: z.object({
                    subChunks: z
                      .array(ChunkDetailSchema)
                      .describe(
                        'The detailed, hierarchical list of sub-chunks.'
                      ),
                  }),
                },
              },
            ...
            if (!planCall) {
              ...
              try {
                ...
                if (
                  parsedPlan.subChunks &&
                  Array.isArray(parsedPlan.subChunks)
                ) {
              ...
              throw new Error(
                `Invalid response from Planner AI. Must call 'submitPlan' tool. Response: ${result.text}`
              );
            }

            // We replace the original high-level chunk with its detailed decomposition
            decomposedChunks.push(
              ...(planCall.input as { subChunks: ChunkDetail[] }).subChunks
            );
            ...
            logVerbose(
              verbose,
              logStream,
              `[WARN] Planner attempt ${attempt} failed for chunk ${topLevelChunk.id}`,
              error
            );
          }
        }
        if (lastError) {
          throw new Error(
            `Planner failed to decompose chunk ${topLevelChunk.id} after ${maxRetries} attempts. Last error: ${lastError.message}`
          );
        }
      }
  ...
  logVerbose(
    verbose,
    logStream,
    '[INFO] Scaffolding files with placeholders...'
  );
  ...
  for (const file of generationState.plan.files) {
    const absoluteFilePath = dir
      ? path.join(dir, file.path, file.fullFileName)
      : path.join(file.path, file.fullFileName);
    ...
    const sortedFileChunkIds = topologicalSort(
      new Map(fileLeafChunks.map((c) => [c.id, c]))
    );

    let scaffoldContent = FILE_TYPES_WITH_IMPORTS.has(file.type)
      ? `${PLACEHOLDER_IMPORTS}\n`
      : '';
    scaffoldContent += sortedFileChunkIds
      .map((id) => CHUNK_PLACEHOLDER(id))
      .join('\n');
  ...
  logVerbose(
    verbose,
    logStream,
    '[INFO] Starting atomic chunk code generation (Developer)...'
  );
  ...
  generationState.plan.files.forEach((file) =>
    collectAllChunksForMap(file.chunks)
  );
  ...
  for (const chunkId of sortedLeafChunkIds) {
    const chunk = Array.from(allChunksMap.values()).find(
      (c) => c.id === chunkId
    )!;
    ...
    logVerbose(
      verbose,
      logStream,
      `[INFO] Generating code for leaf chunk: ${chunk.id}`
    );
...
`;
    ...
    if (prepareSystemPrompt) {
      const preparedSystemPrompt = await prepareSystemPrompt({
        planParams,
        systemPrompt: developerSystemPrompt,
        userPrompt: developerUserPrompt,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const finalDeveloperUserPrompt =
          lastError && attempt > 1
            ? `${developerUserPrompt}\n\n---\nATTEMPT ${attempt - 1} FAILED. Your previous response was invalid.
...
Please correct your response. It MUST be a single call to the 'generateChunk' tool.`
            ...
            generateChunk: {
              ...
              parameters: z.object({
                ...
                imports: z
                  ...
                  .optional()
                  .describe(
                    'A list of imports required by the generated code.'
                  ),
              }),
            },
          },
        ...
        const generateCall = result.toolCalls?.find(
          (t) => t.toolName === 'generateChunk'
        );

        if (generateCall) {
          const { code, imports } = generateCall.input as {
            code: string;
          ...
          logVerbose(
            verbose,
            logStream,
            `[INFO] Generating code for chunk ${chunk.id}.`
          );
          chunk.code = code;
          ...
          if (imports && FILE_TYPES_WITH_IMPORTS.has(file.type)) {
            ...
            if (!generationState.fileImports[absoluteFilePath])
              generationState.fileImports[absoluteFilePath] = {};

            for (const imp of imports) {
              const existing = new Set(
                generationState.fileImports[absoluteFilePath][imp.module] || []
              );
          ...
          throw new Error(
            `Invalid response from developer AI. Must call 'generateChunk'. Response: ${result.text}`
          );
        }
        ...
        logVerbose(
          verbose,
          logStream,
          `[WARN] Attempt ${attempt} failed for chunk ${chunk.id}`,
          error
        );
      }
    }
    if (lastError) {
      throw new Error(
        `Failed to process chunk ${chunk.id} after ${maxRetries} attempts. Last error: ${lastError.message}`
      );
    }
  }
  ...
  for (const file of generationState.plan.files) {
    const absoluteFilePath = dir
      ? path.join(dir, file.path, file.fullFileName)
      : path.join(file.path, file.fullFileName);
    let finalFileContent = await fs.promises.readFile(
      absoluteFilePath,
      'utf-8'
    );
        ...
        for (const modulePath of Object.keys(importsForFile).sort()) {
          ...
          if (specifiers.length > 0) {
            const defaultImport = specifiers.find((s) =>
              s.startsWith('default as')
            );
            const namedImports = specifiers.filter(
              (s) => !s.startsWith('default as')
            );
    ...
    logVerbose(
      verbose,
      logStream,
      `[INFO] Successfully wrote final version of file: ${absoluteFilePath}`
    );
  }
  ...
  for (const file of generationState.plan.files) {
    ...
    try {
      ...
      logVerbose(
        verbose,
        logStream,
        `[WARN] Could not read file for correction: ${absoluteFilePath}`,
        error
      );
      continue;
    }
...
`;
    ...
    try {
      ...
      logVerbose(
        verbose,
        logStream,
        `[INFO] Successfully corrected and overwrote file: ${absoluteFilePath}`
      );
    } catch (error: any) {
      logVerbose(
        verbose,
        logStream,
        `[ERROR] Failed to correct file: ${absoluteFilePath}`,
        error
      );
    }
  }
