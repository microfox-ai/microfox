...
- **Old Tool System**: `router.tool()` has been removed in favor of agent-as-tools
- **Old Tool Calling**: `ctx.next.callTool()` has been removed, use `ctx.next.callAgent()` instead
- **Old Tool Attachment**: `ctx.next.attachTool()` has been removed, use `ctx.next.agentAsTool()` instead
