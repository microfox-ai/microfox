  ...
  for (const pkg of localPackages) {
    ...
    if (isNew || isStale) {
      ...
      upserts.push({
        ...
        oauth2_scopes: pkg.content.oauth2Scopes ? JSON.stringify(pkg.content.oauth2Scopes) : null,
        description: pkg.content.description,
        dependencies: JSON.stringify(pkg.content.dependencies || []),
        added_dependencies: JSON.stringify(pkg.content.addedDependencies || []),
        status: pkg.content.status,
        documentation: pkg.content.documentation,
        icon: pkg.content.icon,
        readme_map: JSON.stringify(pkg.content.readme_map),
        constructors: JSON.stringify(pkg.content.constructors),
        keys_info: JSON.stringify(pkg.content.keysInfo),
        key_instructions: pkg.content.keyInstructions ? JSON.stringify(pkg.content.keyInstructions) : null,
        extra_info: JSON.stringify(pkg.content.extraInfo),
        raw_content: JSON.stringify(pkg.rawContent),
