 ...
 */
function updatePackageInfoReadmeMap(packageName: string): boolean {
...
async function main() {
  ...
  for (const packageName of packagesWithChanges) {
    if (updatePackageInfoReadmeMap(packageName)) {
      updatedCount++;
    }
  }
...
export { updatePackageInfoReadmeMap, getPackagesWithDocsChanges }; 