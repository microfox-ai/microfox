...
async function generateOpenAPIPath(
    ...
    const keysInfo = packageInfo.keysInfo;
    ...
    const schema = z.object({
        ...
        responses: z.record(z.object({
            ...
            })).optional().describe('Response content')
 ...
 */
async function generateOpenAPI(packageInfo: PackageInfo, functionDocs: FunctionDoc[], packageName: string, docContents: Map<string, string>): Promise<any> {
    const openapi = {
        openapi: '3.0.1',
        info: {
    ...
    console.log(`🤖 Generating OpenAPI paths using AI for ${functionDocs.length} functions sequentially...`);
    ...
    for (const funcDoc of functionDocs) {
        ...
        console.log(`🔧 Generating path for: ${funcDoc.name} (${successCount + failureCount + 1}/${functionDocs.length})`);

        try {
            const docContent = docContents.get(funcDoc.name) || '';
            const pathSpec = await generateOpenAPIPath(funcDoc.name, docContent, packageInfo, packageName);
    ...
    console.log(`📊 OpenAPI generation summary: ${successCount} successful, ${failureCount} failed`);

    if (failureCount > 0) {
        console.warn(`⚠️ ${failureCount} functions failed to generate. OpenAPI will be incomplete.`);
    }
 ...
 */
async function generateSDKInitContent(packageInfo: PackageInfo, constructorDocContent: string, templatePath: string): Promise<string> {
    const constructor = packageInfo.constructors[0];
    const functionalities = packageInfo.readme_map?.functionalities?.filter(f => f !== constructor.name) || [];
    const keysInfo = packageInfo.keysInfo;
    const envVars = keysInfo.map(key => key.key);

    // Read the template file
    const template = fs.readFileSync(templatePath, 'utf8');

    const schema = z.object({
        sdkInitCode: z.string().describe('Complete TypeScript code for the sdkInit.ts file'),
    });
...
- Required Keys: ${keysInfo.map(k => `${k.key}: ${k.description}`).join(', ')}
 ...
 */
function updateTemplateFiles(slsDir: string, packageName: string, description: string): void {
    // Update package.json
    ...
    if (fs.existsSync(serverlessPath)) {
        ...
        console.log(`✅ Updated serverless.yml service to: public-${packageName}-api`);
    }
}
...
function updatePackageJson(slsDir: string, packageInfo: PackageInfo): void {
    const packageJsonPath = path.join(slsDir, 'package.json');
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
    const sdkPackageJson = JSON.parse(fs.readFileSync(path.join(slsDir, '..', 'package.json'), 'utf8'));

    // Update name and description
    packageJson.name = 'public-' + packageInfo.name.replace('@microfox/', '') + '-api';
    ...
    try {
        ...
        for (const functionName of functionNames) {
            ...
            if (constructorName && functionName === constructorName) {
                console.log(`⏭️ Skipping constructor function: ${functionName} (constructors don't need OpenAPI endpoints)`);
                skippedCount++;
                continue;
            }
            ...
            if (!fs.existsSync(docPath)) {
                console.error(`❌ Documentation not found for function: ${functionName}`);
                failureCount++;
                continue;
            }
        ...
        console.log(`✅ Updated OpenAPI for ${successCount} functions (${failureCount} failed, ${skippedCount} skipped)`);
 ...
 */
export async function generateSLSStructure(packageName: string, specificFunctions?: string[]): Promise<boolean> {
    try {
        const projectRoot = getProjectRoot();
        ...
        if (specificFunctions && specificFunctions.length > 0) {
            console.log(`🔧 Updating OpenAPI for specific functions: ${specificFunctions.join(', ')} in ${packageName}...`);
        } else {
        ...
        const packageInfo: PackageInfo = JSON.parse(fs.readFileSync(packageInfoPath, 'utf8'));
        const constructor = packageInfo.constructors[0];

        if (!constructor) {
            console.error(`❌ No constructors found in package info for ${packageName}`);
            return false;
        }
        ...
        const functionalities = packageInfo.readme_map?.functionalities?.filter(f => f !== constructor.name) || [];
        console.log(`📋 Found ${functionalities.length} functions: ${functionalities.join(', ')}`);
        ...
        if (specificFunctions && specificFunctions.length > 0) {
            ...
            const filteredFunctions = specificFunctions.filter(func => {
                if (func === constructorName) {
                    console.log(`⏭️ Skipping constructor function: ${func} (constructors don't need OpenAPI endpoints)`);
                    return false;
                }
                return true;
            });
            ...
            if (specificFunctions.length === 0) {
                console.log(`ℹ️ All specified functions were constructors. No OpenAPI updates needed.`);
                return true;
            }

            const invalidFunctions = specificFunctions.filter(func => !functionalities.includes(func));
        ...
        if (specificFunctions && specificFunctions.length > 0 && slsExists) {
            ...
            if (!fs.existsSync(constructorDocPath)) {
                console.error(`❌ Constructor documentation not found: ${constructorDocPath}`);
                return false;
            }
            ...
            const sdkInitContent = await generateSDKInitContent(packageInfo, constructorDocContent, sdkInitTemplatePath);
            ...
            return await updateFunctionsInOpenAPI(packageName, specificFunctions, packageInfo, docsDir);
        }
        ...
        const functionsToProcess = specificFunctions && specificFunctions.length > 0 ? specificFunctions : functionalities;
        ...
        for (const funcName of functionsToProcess) {
            ...
            if (funcName === constructor.name) {
                console.log(`⏭️ Skipping constructor function: ${funcName} (constructors don't need OpenAPI endpoints)`);
                continue;
            }
        ...
        const openapi = await generateOpenAPI(packageInfo, functionDocs, packageName, docContents);
        const openApiPath = path.join(slsDir, 'openapi.json');
        fs.writeFileSync(openApiPath, JSON.stringify(openapi, null, 2));
        ...
        console.error(`❌ Error generating SLS structure for ${packageName}:`, error);
        return false;
    }
}
...
if (require.main === module) {
    ...
    if (!packageName) {
        ...
        console.error('  <package-name>: Name of the package to generate SLS structure for');
        console.error('  [function-names]: Optional. Comma-separated list of function names to update in OpenAPI');
    ...
    (async () => {
        try {
            const success = await generateSLSStructure(packageName, specificFunctions);
